...
export class GeminiAdapter implements IAIService {
  ...
  async getExplanation(userPrompt: string, context: string) {
    try {
      ...
      if (sources.length > 0) {
        ...
        sources.forEach((chunk: any) => {
          if (chunk.web?.uri) {
            text += `- ${chunk.web.title || 'Ver fuente'}\n`;
          }
        });
      }
  ...
  async generateQuestions(subject: string) {
    try {
      const ai = new GoogleGenAI({ apiKey: this.apiKey });
      const response = await ai.models.generateContent({
        model: "gemini-3-pro-preview",
